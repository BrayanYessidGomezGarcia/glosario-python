...
def evaluar_modulo(camper, modulo):
    ...
    nota_practica = float(input("Ingrese la nota práctica del camper para este módulo: "))
    nota_quices = float(input("Ingrese la nota de los quices del camper para este módulo: "))

    # Calcular la nota final con los pesos dados
    nota_final = (nota_teorica * 0.3) + (nota_practica * 0.6) + (nota_quices * 0.1)

    if nota_final >= 60:
        print(f"El camper {camper['NombreCamper']} ha aprobado el módulo {modulo['NombreModulo']} con una nota final de {nota_final}.")
        return True
    else:
        print(f"El camper {camper['NombreCamper']} no ha aprobado el módulo {modulo['NombreModulo']} con una nota final de {nota_final}.")
        return False
